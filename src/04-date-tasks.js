...
function angleBetweenClockHands(/* date */) {
  ...
  const minuteHand = minutes * ANGLE_PER_MINUTE;
  const hourHand = hours * ANGLE_PER_HOUR + (minutes / 60) * ANGLE_PER_HOUR;

  const angle = Math.abs(hourHand - minuteHand);

  return Math.min(360 - angle, angle) === 0
    ? 0
    : Math.min(360 - angle, angle) * (Math.PI / 180); */
